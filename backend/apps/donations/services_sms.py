...
class SMSDonationService:
    ...
    def send_confirmation_sms(cls, donation, phone_number):
        """Send an SMS confirmation after a donation via Twilio."""
        try:
            ...
            if not all([twilio_sid, twilio_token, twilio_from]):
                logger.info(
                    'Twilio not configured. SMS confirmation skipped for '
                    'donation %s', donation.donation_number,
                )
                return False
            ...
            logger.info('SMS confirmation sent for donation %s', donation.donation_number)
            return True

        except ImportError:
            logger.warning('twilio package not installed -- SMS confirmation skipped.')
            return False
        except Exception as e:
