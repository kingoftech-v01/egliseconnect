...
class HelpRequestCategoryViewSet(RateLimitHeadersMixin, viewsets.ReadOnlyModelViewSet):
    queryset = HelpRequestCategory.objects.filter(is_active=True)
    serializer_class = HelpRequestCategorySerializer
    permission_classes = [IsAuthenticated]


class HelpRequestViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
    ...
    def comments(self, request, pk=None):
        ...
        if page is not None:
            serializer = HelpRequestCommentSerializer(page, many=True)
            return self.get_paginated_response(serializer.data)
...
class PastoralCareViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
...
class PrayerRequestViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
    ...
    def wall(self, request):
        ...
        if page is not None:
            serializer = PrayerRequestSerializer(page, many=True)
            return self.get_paginated_response(serializer.data)
...
class CareTeamViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
...
class CareTeamMemberViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
...
class BenevolenceFundViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
    serializer_class = BenevolenceFundSerializer
    permission_classes = [IsPastor | IsAdmin]
    queryset = BenevolenceFund.objects.all()


class BenevolenceRequestViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
...
class MealTrainViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
...
class MealSignupViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
...
class CrisisProtocolViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
...
class CrisisResourceViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
