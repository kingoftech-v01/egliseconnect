...
def send_newsletter_task(newsletter_id):
    ...
    if emails:
        ...
        messages = [
            (newsletter.subject, newsletter.body, from_email, [email])
            for email in emails
        ]
        try:
            send_mass_mail(messages, fail_silently=False)
            logger.info("Newsletter '%s' sent to %d recipients.", newsletter.subject, len(emails))
        except Exception as exc:
            logger.error("Newsletter '%s' send failed: %s", newsletter.subject, exc)

    newsletter.status = NewsletterStatus.SENT
    newsletter.sent_at = timezone.now()
    newsletter.save(update_fields=['status', 'sent_at', 'updated_at'])

    return f"Newsletter {newsletter_id} sent to {len(emails)} recipients."
