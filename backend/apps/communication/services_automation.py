...
class AutomationService:
    ...
    def _send_email(self, step, member):
        """Send an email for an automation step."""
        from django.core.mail import send_mail
        from django.conf import settings

        if not member.email:
            logger.warning("No email for member %s, skipping email step.", member.pk)
            return
        ...
        try:
            ...
            logger.info("Automation email sent to %s: %s", member.email, step.subject)
        except Exception as exc:
            logger.error("Automation email failed to %s: %s", member.email, exc)
